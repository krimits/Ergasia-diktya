...
Οι τρεχουσες τιμες συμφωνουν με την εκφωνηση (παραδειγμα ~1 λεπτο / RAND*120 sec):
- `AUCTION_POLL_INTERVAL = 60` (getCurrentAuction)
- `ITEM_GEN_MAX_INTERVAL = 120` (παραγωγη αντικειμενων)
...
- **Ονοματα μηνυματων JSON:** Στον κωδικα χρησιμοποιουνται τυποι οπως `GET_CURRENT_AUCTION`, `REQUEST_AUCTION` (αντι για τα λεκτικα getCurrentAuction/requestAuction της εκφωνησης). Η συμπεριφορα ειναι ισοδυναμη.
- **Λοιπα:** Μετα το login στελνεται παντα `REQUEST_AUCTION` (και με κενη λιστα αντικειμενων) ωστε να καταχωρουνται `ip_address`/`port` για καθε συνδεδεμενο peer (§4.3.b, §5.3, §5.7). Ο server απορριπτει προσφορα πωλητη στο δικο του αντικειμενο.
...
```
[sotiris] -> REGISTER username=sotiris password=pass123
[SERVER] REGISTER  sotiris
[sotiris] <- success=True  msg='Registered successfully.'

[sotiris] -> REGISTER username=sotiris (duplicate)
[sotiris] <- success=False  msg='Username already taken. Choose another.'

[trifonas]   -> REGISTER username=trifonas password=trifonaspass
[SERVER] REGISTER  trifonas
[trifonas]   <- success=True  msg='Registered successfully.'
```
...
```
[sotiris] -> LOGIN username=sotiris password=pass123
[SERVER] LOGIN     sotiris  token=264246
[sotiris] <- success=True  token=264246  msg='Login successful.'

[sotiris] -> LOGIN again (duplicate session)
[sotiris] <- success=False  msg='Already logged in.'
```
...
```
[trifonas]   -> LOGOUT token=799875
[SERVER] LOGOUT    trifonas
[trifonas]   <- success=True  msg='Logged out.'

[trifonas]   -> LOGOUT again (invalid token)
[trifonas]   <- success=False  msg='Invalid token.'
```
...
```
[sotiris] -> REQUEST_AUCTION  Object_A_01 (Vintage Watch, bid=50, dur=25s)
[SERVER] QUEUED    Object_A_01  from sotiris
[SERVER] AUCTION   Object_A_01 | Vintage Watch
[SERVER]           start_bid=50.00  duration=25s  seller=sotiris

[trifonas]   -> GET_CURRENT_AUCTION
[trifonas]   <- active=True  object_id=Object_A_01  description=Vintage Watch

[trifonas]   -> GET_AUCTION_DETAILS
[trifonas]   <- highest_bid=50.00  seller_token=264246  remaining=22.0s
```

### 4.5 Επιτυχης προσφορα πλειοδοσιας

```
[trifonas]   -> PLACE_BID Object_A_01 bid=52.50
[SERVER] BID       52.50  by trifonas  on Object_A_01
[trifonas]   <- success=True  msg='Bid accepted.'
[sotiris] notification -> new bid on Object_A_01: 52.50 by trifonas
[carol] notification -> new bid on Object_A_01: 52.50 by trifonas
[dave]  notification -> new bid on Object_A_01: 52.50 by trifonas
...
```
[SERVER] ENDED     Object_A_01  won by dave  for 60.00  (seller: sotiris)
[dave]  *** WON auction Object_A_01 for 60.00 ***
[sotiris] *** SOLD Object_A_01 for 60.00 to dave ***
```
...
```
[sotiris] Sold Object_A_01 -> file transferred & removed
...
[sotiris] seller file Object_A_01.txt exists = False (should be False)
[dave]  buyer  file Object_A_01.txt exists = True  (should be True)
```

Ο αγοραστης (dave) συνδεεται P2P στον πωλητη (sotiris), λαμβανει το αρχειο
...
[trifonas]   -> PLACE_BID Object_E_01 bid=35.00
[SERVER] BID       35.00  by trifonas  on Object_E_01
[trifonas]   <- Bid accepted.

>>> [eve] DISCONNECTING (simulating seller crash) <<<

[SERVER] CANCELLED Object_E_01  reason: Seller disconnected
[trifonas]   AUCTION CANCELLED: Object_E_01 reason=Seller disconnected
```
...
```
[SERVER] BID       52.50  by trifonas  on Object_A_01
[trifonas]   notification -> new bid on Object_A_01: 52.50 by trifonas
[dave]  notification -> new bid on Object_A_01: 52.50 by trifonas
[sotiris] notification -> new bid on Object_A_01: 52.50 by trifonas
[carol] notification -> new bid on Object_A_01: 52.50 by trifonas
[eve]   notification -> new bid on Object_A_01: 52.50 by trifonas
```
